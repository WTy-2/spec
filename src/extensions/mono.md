...
A variable can either be annotated with a `mono` or a `poly` mode with `mono <: poly` subtyping relation. Informally, `mono` represents that the value of a variable can be computed at compile-time.
