...
- Variables, `v : t = e`, and their types (`v : t`) can be exported/imported via their identifier. The RHS of their definition (`v ~ e`) can be exported/imported with `(..)`.
...
- Tags can be exported via their identifiers. Modifying the type of arguments to a tag is always a breaking change.
